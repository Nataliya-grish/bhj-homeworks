    ...
    if (event.target.classList.contains("product__add")) {
      for (let j = 0; j < productsInCart.children.length; j++) {
        if (productsInCart.children[j].dataset.id === products[i].dataset.id) {
          productsInCart.children[j].querySelector(
            ".cart__product-count"
          ).textContent =
            Number(
              productsInCart.children[j].querySelector(".cart__product-count")
                .textContent
            ) +
            Number(
              products[i].querySelector(".product__quantity-value").textContent
            );
          return;
        }
      }
      ...
      const quantity = products[i].querySelector(".product__quantity-value")
        .textContent;
